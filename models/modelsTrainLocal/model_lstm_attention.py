...
from tensorflow.keras.layers import LSTM, Dense, Dropout, Attention, Input,GlobalAveragePooling1D
...
class Model_lstm_attention:
    ...
    def sequence_model(self, df_cmin3_valid_model, end_date, params):
        ...
        x = LSTM(params["hidden_units_1"], return_sequences=True, kernel_regularizer=l2(0.01), activity_regularizer=l1_l2(0.01, 0.01))(inputs)
        x = Dropout(params["dropout_rate"])(x)

        x = LSTM(params["hidden_units_2"], return_sequences=True, kernel_regularizer=l2(0.001), activity_regularizer=l1_l2(0.001, 0.001))(x)
        x = Dropout(params["dropout_rate2"])(x)

        x = LSTM(params["hidden_units_3"], return_sequences=True)(x)
        x = Dropout(params["dropout_rate3"])(x)

        # Mecanismo de Atenção
        query = x  # Última camada LSTM
        value = x  # Todos os estados ocultos
        attention_out = Attention()([query, value])

        pooling_layer = GlobalAveragePooling1D()(attention_out)

        # Saída Final
        output = Dense(1)(pooling_layer)
